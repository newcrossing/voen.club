 ...
 * $db->query('SELECT ?i', 55.5); // Попытка указать для заполнителя типа int значение типа double в шаблоне запроса
 * SELECT ?i
 *
 ...
 *
 * ?a* - заполнитель множества из простого (или также ассоциативного) массива-аргумента, генерирующий
 * последовательность
 *       значений.
 *       Пример: "val_1", "val_2", ...
 *
...
class Mysql
{
    ...
    private function parse($query, array $args, $original_query = null)
    {
        ...
        while (($posQM = mb_strpos($query, '?', $offset)) !== false) {
            ...
            if ($placeholder_type == '' || !in_array($placeholder_type,
                            array('i', 'd', 's', 'S', 'n', 'A', 'a', 'f'))) {
                $offset += 1;
                continue;
            }
            ...
            switch ($placeholder_type) {
                ...
                case 'a':
                    ...
                    if ($next_char != '' && preg_match('#[sid\[]#u', $next_char, $matches)) {
                        ...
                        elseif (preg_match('#[sid]#u', $next_char, $matches)) {
                            $sql = '';
                            $parts = array();
